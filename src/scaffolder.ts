...
- Socket Security policy scan - runs in \`.github/workflows/socket.yml\` when \`SOCKET_SECURITY_API_KEY\` is configured in GitHub; \`SOCKET_ORG\` can override the default lowercased GitHub repository owner slug
...
- CI includes an optional pinned Socket Security scan when \`SOCKET_SECURITY_API_KEY\` is configured in GitHub. \`SOCKET_ORG\` can override the default lowercased GitHub repository owner slug when the Socket organization slug differs from the GitHub owner.
