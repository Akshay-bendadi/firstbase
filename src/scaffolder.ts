...
- Socket Security policy scan - runs in \`.github/workflows/socket.yml\` when \`SOCKET_SECURITY_API_KEY\` and \`SOCKET_ORG\` are configured in GitHub; \`SOCKET_ORG\` must be the Socket organization slug
...
- CI includes an optional pinned Socket Security scan when \`SOCKET_SECURITY_API_KEY\` and \`SOCKET_ORG\` are configured in GitHub. \`SOCKET_ORG\` must be the Socket organization slug, not a defaulted GitHub owner or repository name.
