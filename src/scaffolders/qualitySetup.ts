...
jobs:
  socket:
    ...
    steps:
      ...
      - name: Resolve Socket organization
        id: socket-org
        if: steps.socket-config.outputs.enabled == 'true'
        shell: bash
        run: |
      ...
      - name: Run Socket policy scan
        ...
        run: |
          ...
          socket scan create --org "\${{ steps.socket-org.outputs.org }}" --repo "$repo" --branch "$branch" --report --no-interactive .
`;
}
