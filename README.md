...
- Adds an optional Socket Security workflow when `SOCKET_SECURITY_API_KEY` is configured in GitHub; `SOCKET_ORG` can override the Socket CLI default organization when needed
...
- Generated Socket Security workflow always runs a preflight job, skips cleanly when the Socket token is not configured, and lets the Socket CLI use the token's default organization. Set the repository variable `SOCKET_ORG` only when you need to force a specific Socket organization slug.
