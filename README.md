...
- Adds an optional Socket Security workflow when `SOCKET_SECURITY_API_KEY` is configured in GitHub; it discovers the Socket organization from the token and supports `SOCKET_ORG` as an override
...
- Generated Socket Security workflow always runs a preflight job, skips cleanly when the Socket token is not configured, discovers the Socket organization from the API token, and passes that slug to the CLI scan. Set the repository variable `SOCKET_ORG` only when you need to force a specific Socket organization slug.
