...
- CI can run Socket Security policy scans when `SOCKET_SECURITY_API_KEY` is configured. The workflow discovers the Socket organization from the API token, and `SOCKET_ORG` can override it when needed.
